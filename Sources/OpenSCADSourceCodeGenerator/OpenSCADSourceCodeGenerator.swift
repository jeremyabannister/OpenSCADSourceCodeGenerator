...
public struct OpenSCADSourceCodeGenerator {
  public static func sourceCode (for object: Object3DEnum) -> String {
    return OpenSCADSourceCodeGenerator.sourceCode(for: object, shouldAddVariables: true)
  }
  
  internal static func sourceCode (for object: Object3DEnum, shouldAddVariables: Bool) -> String {
    ...
    case .union(position: let position, objectEnums: let objectEnums):
      let translation = OpenSCADSourceCodeGenerator.translationString(for: position)
      let interiorSourceCode = objectEnums.map({ OpenSCADSourceCodeGenerator.sourceCode(for: $0, shouldAddVariables: false) }).joined(separator: "\n")
